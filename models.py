...
class Dataset(ProxyModel):
    ''' Metadata for a datset from a CKAN repository. '''

    # These two fields can help us to group the results of a
    # specific round of data fetching.
    fetch_index = IntegerField()
    date = DateTimeField(index=True, default=datetime.datetime.now)

    # This ID is the 'id' of the original record
    dataset_id = CharField(max_length=36, index=True)
    license_title = CharField(null=True)
    title = CharField()


class Resource(ProxyModel):
    ''' A resource (file) with the data for a datset. '''
